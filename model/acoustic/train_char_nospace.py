...
name_regex = r"^densenet_char_nospace_b([0-9]+)\.l([0-9]+)\.f([0-9]+)\.k([0-9]+)\.([a-z]+)"
...
if not os.path.isfile("saved_models/"+model_name):
    ...
    for i in range(n_blocks):
        r = res_block(r)
    logits =  BatchNormalization()(Conv1D(n_chars, 
                                         kernel_size,
                                         padding='same', 
                                         dilation_rate=1, 
                                         activation="linear")(r))
    ...
    loss_out = Lambda(ctc_lambda_func, output_shape=(1,), name='ctc')([char_probs, labels_masked, input_length, label_length])

    # Create the model and compile
    model = Model(inputs=[log_mfes, labels, input_length, label_length], outputs=[loss_out, char_probs])
    #model.summary()
    ...
    model, highest_epoch = load_model_char(model_name, mode="latest")
    '''
...
train_data_gen = batch_generator_char(batch_size, 'train_char', n_chars, max_label_len, max_input_len, max_ratio, aug=True)
valid_data_gen = batch_generator_char(batch_size, 'valid_char', n_chars, max_label_len, max_input_len, max_ratio, aug=False)

train_batches=0
n_batches = {}
for set_name in ['train_char', 'valid_char']:
...
history=model.fit_generator(train_data_gen, 
                            steps_per_epoch = n_batches['train_char'], 
                            epochs=n_epochs, 
                            verbose = 1, 
                            callbacks=[checkpoint_callback, log_callback],
                            validation_data = valid_data_gen, 
                            validation_steps=n_batches['valid_char'])
