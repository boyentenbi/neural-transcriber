...
def process_vctk(csv_file):
    ...
    print "Starting VCTK word labelling.."
    ...
    for i, f in enumerate(file_ids):
        ...
        # get label index
        word_label = str2wordlabel(open(_data_path + 'VCTK-Corpus/txt/%s/' % f[:4] + f + '.txt').read())
        ...
        if word_label and len(word_label) < mfcc.shape[0]:
            # save meta info
            writer.writerow([fn] + word_label)
...
def process_libri(csv_file, category):
    print "Starting LibriSpeech/{} word labelling...".format(category)
            ...
            for txt in txt_list:
                with open(txt, 'rt') as f:
                    ...
                    for record in records:
                        ...
                        # label index
                        labels.append(str2wordlabel(' '.join(field[1:])))
    ...
    for stm in stm_list:
        with open(stm, 'rt') as f:
            ...
            for record in records:
                ...
                labels.append(str2wordlabel(uncleaned_label))
...
# VCTK corpus
csv_f = open('asset/data/preprocess/meta/train_word.csv', 'w')
process_vctk(csv_f)
csv_f.close()

csv_f = open('asset/data/preprocess/meta/train_word.csv', 'r')
reader = csv.reader(csv_f, delimiter=",")
print "train_word.csv has {} rows currently".format(len([x for x in reader]))
csv_f.close()
                                               
# LibriSpeech corpus for train
csv_f = open('asset/data/preprocess/meta/train_word.csv', 'a')
...
csv_f = open('asset/data/preprocess/meta/valid_word.csv', 'w')
process_libri(csv_f, 'dev-clean')
csv_f.close()

csv_f = open('asset/data/preprocess/meta/valid_word.csv', 'r')
reader = csv.reader(csv_f, delimiter=",")
print "valid_word.csv has {} rows currently".format(len([x for x in reader]))
csv_f.close()   

# TEDLIUM corpus for valid
csv_f = open('asset/data/preprocess/meta/valid_word.csv', 'a')
...
csv_f = open('asset/data/preprocess/meta/test_word.csv', 'w')
process_libri(csv_f, 'test-clean')
csv_f.close()

csv_f = open('asset/data/preprocess/meta/test_word.csv', 'r')
reader = csv.reader(csv_f, delimiter=",")
print "test_word.csv has {} rows currently".format(len([x for x in reader]))
csv_f.close()   

# TEDLIUM corpus for test
csv_f = open('asset/data/preprocess/meta/test_word.csv', 'a')
...
for set_name in ["train_word", "valid_word", "test_word"]:
#     set_name = "train"
    # Get the labels and 
