...
if not os.path.isfile("saved_models/"+model_name):
    ...
    for k in filenames:
        ...
        if match:
            ...
            epochs_done = match.group(1)
            val_loss = match.group(2)
            print "File", k, "contains weights achieving validation loss:", val_loss
            # If this is the first file, or if the val loss is better, change the lowest loss and the best file
            if not lowest_loss or val_loss < lowest_loss:
                lowest_loss = min(val_loss, lowest_loss)
                best = k
            else:
                continue
    if best:
        print "Best weights are from", best, "with validation loss", val_loss
