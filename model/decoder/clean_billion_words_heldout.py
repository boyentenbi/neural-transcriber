...
folder_name = "language_model_dat/1-billion-word/heldout"
f_names = os.listdir(folder_name)
text_names = [os.path.join(folder_name, f_name) for f_name in f_names if re.match(r"^news.en.heldout-([0-9]+)-of-00050$", f_name)]
...
#     cleaned_sentence = map(lambda x : x if x in allowed_chars else ' ', sentence.lower())
    return (" ".join(x_list) + "\n")
print "Starting cleaning of billion words (heldout)..."
f = open("language_model_dat/processed-text-clean-v2-heldout.txt", "w")
